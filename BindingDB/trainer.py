...
class Trainer(object):
    ...
    def train(self):
        ...
        #scheduler = ExponentialLR(self.optim, gamma=0.5)  # 定义指数衰减学习率调度器
        for i in range(self.epochs): #训练epochs
        #     if i % 250 == 0 and i != 0:   # 在每个epoch结束后，如果是第20个epoch，则更新学习率
        #         scheduler.step()
    ...
    def train_epoch(self):  #训练一轮
        alpha_net = 0.001 #超参调节 0.001
        alpha_node = 0.001 #超参调节 0.001
        alpha_edge = 0.001
        self.model.train()
        loss_epoch = 0
        ...
        for i, (v_d, v_p, labels) in enumerate(tqdm(self.train_dataloader)):
            ...
            v_d, v_p, f, score, loss_con_net, loss_con_edge, loss_con_node = self.model(v_d, v_p)
            if self.n_class == 1:
                n, loss1 = binary_cross_entropy(score, labels)
                loss = loss1 + alpha_net*loss_con_net + alpha_edge*loss_con_edge + alpha_node*loss_con_node
            else:
                n, loss1 = cross_entropy_logits(score, labels)
                loss = loss1 + alpha_net*loss_con_net + alpha_edge*loss_con_edge + alpha_node*loss_con_node
            loss.backward()
            self.optim.step()
    ...
    def train_da_epoch(self):
        ...
        for i, (batch_s, batch_t) in enumerate(tqdm(self.train_dataloader)):
            ...
            v_d, v_p, f, score, _, _, _ = self.model(v_d, v_p)
            if self.n_class == 1:
                n, model_loss = binary_cross_entropy(score, labels)
            else:
                n, model_loss = cross_entropy_logits(score, labels)
            if self.current_epoch >= self.da_init_epoch:
                v_d_t, v_p_t, f_t, t_score, _, _, _ = self.model(v_d_t, v_p_t)
        ...
        with torch.no_grad():
            ...
            for i, (v_d, v_p, labels) in enumerate(data_loader):
                ...
                if dataloader == "val":
                    ...
                    v_d, v_p, f, score, loss_con_net, loss_con_edge, loss_con_node = self.best_model(v_d, v_p)
                if self.n_class == 1:
                    n, loss1 = binary_cross_entropy(score, labels)
                    loss = loss1 #+ 0*loss_con_net
                else:
                    n, loss1 = cross_entropy_logits(score, labels)
                    loss = loss1 #+ 0*loss_con_net
                test_loss += loss.item()
