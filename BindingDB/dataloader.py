...
class DTIDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        v_d = self.df.iloc[index]['SMILES'] #获取SMILES这一列的数据  #df表示DataFrame  df.iloc[a, b] a表示行索引；b表示列索引
        v_d = self.fc(smiles=v_d, node_featurizer=self.atom_featurizer, edge_featurizer=self.bond_featurizer)  #将序列转换为图
        actual_node_feats = v_d.ndata.pop('h')  #.ndata获取某些节点特征  .pop('h'):为了删除‘h’
        ...
        v_p = integer_label_protein(v_p) #编码protein
